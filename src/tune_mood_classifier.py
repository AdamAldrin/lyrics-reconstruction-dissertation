...
from run_mood_classifier_experiments import aggregate_metrics, resolve_path, run_single_seed
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Tune TF-IDF logistic regression for the 12-mood classifier.")
    parser.add_argument("--data-dir", default="data/processed/mood_classifier")
    parser.add_argument("--output-dir", default="outputs/mood_classifier_experiments/12_mood_logreg_tfidf_tuning")
...
def main() -> None:
    ...
    config_rows = []
    all_split_rows = []
    configs = list(product(args.max_features, args.ngram_max, args.logreg_c))

    for config_index, (max_features, ngram_max, logreg_c) in enumerate(configs, start=1):
        config_name = f"features_{max_features}_ngram_{ngram_max}_c_{logreg_c:g}"
    ...
    print(f"\nSaved tuning outputs to: {output_dir}")
